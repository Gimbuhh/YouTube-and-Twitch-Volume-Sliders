// ==UserScript==
// @name         Twitch Volume Slider 2.4.1
...
(() => {
  ...
  function createVolumeSettings({
    ...
    function isOverlayInteractionFocused(overlay) {
      return overlay?.dataset.tmDragging === "true" || overlay?.matches?.(":hover") || overlay?.contains?.(document2.activeElement);
    }
  ...
  var focusableSelector = 'input:not(:disabled),button:not(:disabled),select:not(:disabled),textarea:not(:disabled),[tabindex]:not([tabindex="-1"])';
